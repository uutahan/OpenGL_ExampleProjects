...
class Torus(Object):
    ...
    def increase_subdivisions(self):
        self.configure(self.longs+1,self.lats+1,self.radius,self.bigRadius)
    ...
    def configure(self,longs,lats,radius,bigRadius):
        self.longs=longs
        self.lats=lats
        self.radius=radius
        self.bigRadius=bigRadius
        
        self.points=self.get_points(self.longs,self.lats,self.radius,self.bigRadius)
        self.matrix=self.get_matrix(self.points)
        self.center=self.get_center(self.points)
        self.apply_transformation_matrix()
        
    def tr_point(self,longAngle,latAngle,radius,bigRadius):
        return Position((bigRadius+cos(latAngle)*radius)*cos(longAngle),
                        (bigRadius+cos(latAngle)*radius)*sin(longAngle),
                        radius*sin(latAngle)) 

    def get_points(self,longCount,latCount,radius,bigRadius):
        points=[]
        ...
        for i in range(longCount): 
             for j in range(latCount): 
                 ...
                 p0=self.tr_point(longAngle, latAngle,radius,bigRadius)
                 p1=self.tr_point(longAngle, latAngleNext,radius,bigRadius)
                 p2=self.tr_point(longAngleNext, latAngle,radius,bigRadius)
                 p3=self.tr_point(longAngleNext, latAngleNext,radius,bigRadius)
